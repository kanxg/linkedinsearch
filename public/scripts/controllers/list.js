	...
	function ListCtrl($scope, $http, $rootScope, goodsOrderActiveClass, goodsViewActiveClass){
		...
		var getShopList = function(callback){
			...
			var para = {
				  "query":  { 
    		            "match" : { 
    		            	"_all": $scope.searchWords||$rootScope.searchWords||"" 
                	  }
            	 },
				 "aggs": {
					"connections": {
			...
			$.ajax({  
				...
				success:function(data){
					$scope.shopList=[];
					for(var i = 0;i<data.aggregations.connections.buckets.length;i++){
						$scope.shopList.push({'name':data.aggregations.connections.buckets[i].key,'class':$scope.getShopClass(data.aggregations.connections.buckets[i].key,true)});
					}
					$scope.$apply();
				}
			});  
		...
		var getCateList = function(callback){	
			...
			$.ajax({  
				...
				success:function(data){  
					$scope.cateList=[];
					for(var i = 0;i<data.aggregations.locality.buckets.length;i++){
							$scope.cateList.push($.extend(data.aggregations.locality.buckets[i],{'class':''}));
					}
					$scope.$apply();
				}  
			});  
